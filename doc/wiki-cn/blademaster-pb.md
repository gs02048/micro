...
`micro tool protoc`工具可以生成`warden` `bm` `swagger`对应的代码和文档，想要单独生成`bm`代码只需加上`--bm`如：
...
service Demo {
	...
	rpc SayHelloURL(HelloReq) returns (HelloResp) {
        option (google.api.http) = {     // 该配置指定SayHelloURL方法对应的url
            get:"/micro-demo/say_hello" // 指定url和请求方式为GET
        };
    };
}
```
...
使用`RegisterDemoBMServer`示例代码请参考[micro-demo内的http](https://github.com/gs02048/micro-demo/blob/master/internal/server/http/server.go)内的如下代码：
...
`internal/service`内的`Service`结构实现了`DemoBMServer`接口可参考[micro-demo内的service](https://github.com/gs02048/micro-demo/blob/master/internal/service/service.go)内的如下代码：
...
```shell
micro tool swagger serve api/api.swagger.json
```
