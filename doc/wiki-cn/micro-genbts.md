...
在internal/dao/dao.go中添加mc缓存interface定义，可以指定对应的[注解参数](../../tool/micro-gen-bts/README.md)；
并且在接口前面添加`go:generate micro tool genbts`；
然后在当前目录执行`go generate`，可以看到自动生成的dao.bts.go代码。  

### 回源模板
```go
//go:generate micro tool genbts
type _bts interface {
...
也可以参考完整的testdata例子：micro/tool/micro-gen-bts/testdata
