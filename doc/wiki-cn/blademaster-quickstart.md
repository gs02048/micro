...
func initRouter(e *bm.Engine) {
	...
	g := e.Group("/micro-demo") // e.Group 创建一组 "/micro-demo" 起始的路由组
	{
		g.GET("/start", howToStart) // g.GET 创建一个 "micro-demo/start" 的路由，使用GET方式请求，默认处理Handler为howToStart方法
		g.POST("start", howToStart) // g.POST 创建一个 "micro-demo/start" 的路由，使用POST方式请求，默认处理Handler为howToStart方法
	}
}
```
...
func pathParam(c *bm.Context) {
	...
	path := c.RoutePath // NOTE: 获取注册的路由原始地址，如: /micro-demo/param1/:name
