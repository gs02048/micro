...
以下工具已加入到 micro 框架 go modules，可以借助 testgen 代码生成器自动生成部分工具代码，请放心食用。更多使用方法还欢迎大家多多探索。
...
#### 使用建议
强烈建议使用 [testgen](https://github.com/gs02048/micro/blob/master/doc/wiki-cn/ut-testgen.md) 进行测试用例的生成，生成后每个方法将包含一个符合以下规范的正向用例。
...
## 示例：
mockgen -destination=$GOPATH/micro/app/xxx/dao/dao_mock.go -package=dao micro/app/xxx/dao DaoInterface
```
