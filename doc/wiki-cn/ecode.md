...
在`micro`里，错误码被设计成`Codes`接口，声明如下[代码位置](https://github.com/gs02048/micro/blob/master/pkg/ecode/ecode.go)：
...
1. `ecode`包内的`Status`结构体实现了`Codes`接口[代码位置](https://github.com/gs02048/micro/blob/master/pkg/ecode/status.go)
2. `warden/internal/status`包内包装了`ecode.Status`和`grpc.Status`进行互相转换的方法[代码位置](https://github.com/gs02048/micro/blob/master/pkg/net/rpc/warden/internal/status/status.go)
3. `warden`的`client`和`server`则使用转换方法将`gRPC`底层返回的`error`最终转换为`ecode.Status` [代码位置](https://github.com/gs02048/micro/blob/master/pkg/net/rpc/warden/client.go#L162)
...
使用`micro tool protoc --ecode user.proto`进行生成，生成如下代码：

```go
package ecode

import (
    "github.com/gs02048/micro/pkg/ecode"
)
